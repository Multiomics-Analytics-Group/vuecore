...
@document_pydant_params(ScatterMapConfig)
def create_scatter_map_plot(
    data: pd.DataFrame,
    ...
    **kwargs,
    ...
    """
    Creates, styles, and optionally saves a scatter map plot using the specified engine.

    This function serves as the main entry point for users to generate scatter map plots.
    It validates the provided configuration against the `ScatterMapConfig` schema,
    ...
    * **Python Script:** `docs/api_examples/scatter_plot.py` -
    ...
    return create_plot(
        ...
        plot_type=PlotType.SCATTER_MAP,
        engine=engine,
        file_path=file_path,
        **kwargs,
    )
